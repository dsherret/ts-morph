...
export interface GetTextChangingChildOrderOptions<TParentNode extends Node> {
    parent: TParentNode;
    oldIndex: number;
    newIndex: number;
...
export function getTextChangingChildOrder<TParentNode extends Node>(opts: GetTextChangingChildOrderOptions<TParentNode>) {
    const {parent, oldIndex, newIndex, getSiblingFormatting} = opts;
