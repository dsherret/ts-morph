...
copyDeclarationsForDeno(`./lib/ts-morph-common.d.ts`, `${copyDirPath}/ts_morph_common.d.ts`);
...
/**
 * Copies a declaration file, adapting it to what Deno and JSR accept.
 *
 * Two things change. Every relative specifier gains an extension, because Deno
 * resolves a specifier as written where node and a bundler would try extensions
 * for it — `"../enums/scriptKind.enum"` has to become
 * `"../enums/scriptKind.enum.d.ts"`. And the `esnext.disposable` lib reference
 * the vendored client carries is dropped, because JSR rejects a triple-slash
 * directive that modifies globals; Deno's own default library already declares
 * `Symbol.dispose`.
 */
function copyDeclarationsForDeno(fromPath: string, toPath: string) {
  ...
  fileSystem.writeFileSync(toPath, addExtensions(dropLibReferences(text)));
}

function dropLibReferences(text: string) {
  return text.replace(/^\/\/\/\s*<reference\s+lib=[^>]*\/>\r?\n/gm, "");
}

/**
 * Gives every relative specifier a `.d.ts` extension.
 *
 * `from` and `import` are matched on a word boundary so an identifier ending in
 * one is left alone. A specifier that already names a file is left as it is —
 * which cannot be decided by looking for a dot, because these file names have
