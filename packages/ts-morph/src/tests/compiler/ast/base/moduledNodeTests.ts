  ...
  describe(nameof<ModuledNode>("insertImportDeclarations"), () => {
    ...
    it("should support writing named imports with a writer", () => {
      doTest(
        ``,
        0,
        [{ namedImports: writer => writer.writeLine("test,").writeLine("test2"), moduleSpecifier: "./test" }],
        `import { test,\n    test2\n} from "./test";\n`,
      );
    });
  ...
  describe(nameof<ModuledNode>("getExportedDeclarations"), () => {
    function assertMapsEqual(expected: [string, string[]][], actual: ReadonlyMap<string, ExportedDeclarations[]>) {
      ...
      function assertExportedDeclaration(node: ExportedDeclarations) {
        if (
          !Node.isClassDeclaration(node) && !Node.isInterfaceDeclaration(node) && !Node.isEnumDeclaration(node) && !Node.isFunctionDeclaration(node)
    ...
    it("should get the namespace import identifier of one that's exported from an imported namespace export that imports a namespace declaration", () => {
      ...
      fileSystem.writeFileSync(
        "/node_modules/@types/typescript/index.d.ts",
        `
